...
// Updated: saves profile to backend + matches schemes via API
import { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { Mic, MicOff, Send, Bot, User, Globe, X } from 'lucide-react';
...
const COMPLETION_MSG: Record<LangCode, string> = {
  hi: '🎉 शुक्रिया! आपका प्रोफ़ाइल सुरक्षित हो गया है। अब आपके लिए योजनाएं खोज रहे हैं...',
  en: '🎉 Thank you! Your profile has been saved. Finding government schemes for you...',
  ta: '🎉 நன்றி! உங்கள் சுயவிவரம் சேமிக்கப்பட்டது. திட்டங்கள் தேடுகிறேன்...',
  te: '🎉 ధన్యవాదాలు! మీ ప్రొఫైల్ సేవ్ అయింది. పథకాలు వెతుకుతున్నాను...',
  bn: '🎉 ধন্যবাদ! আপনার প্রোফাইল সংরক্ষিত হয়েছে। প্রকল্প খুঁজছি...',
  mr: '🎉 धन्यवाद! तुमचे प्रोफाइल जतन झाले. योजना शोधत आहे...',
};
...
const WelcomeBot = () => {
  ...
  // TTS: Sarvam AI → fallback browser
  ...
  const addBotMessage = (text: string, extra?: Partial<ChatMessage>) => {
    setMessages(prev => [...prev, { id: Date.now().toString(), role: 'bot', text, ...extra }]);
  };
  ...
  const processAnswer = async (answerText: string) => {
    ...
    if (nextStep < questions.length) {
      ...
      if (nextQ.type === 'yesno') {
        addBotMessage(nextQ.text, { type: 'options', options: [
          { label: YES_NO_LABELS[lang].yes, value: 'yes' },
          { label: YES_NO_LABELS[lang].no, value: 'no' },
        ]});
      } else { addBotMessage(nextQ.text); }
      speak(nextQ.text);
  ...
  const saveToBackend = async (finalAnswers: Record<string, string>) => {
    const rawProfile = {
      name: finalAnswers.name || 'User',
      district: finalAnswers.location?.split(',')[0]?.trim() || finalAnswers.location || '',
      state: finalAnswers.location?.split(',')[1]?.trim() || '',
      occupation: finalAnswers.occupation || '',
      family_size: parseInt(finalAnswers.family_size) || 4,
      monthly_income: parseInt(finalAnswers.monthly_income?.replace(/[^\d]/g, '')) || 10000,
    ...
    try {
      ...
      const fullProfile = { ...rawProfile, id: userId, created_at: new Date().toISOString(), schemes_matched: schemes.length };
      ...
      const fallback = { ...rawProfile, id: crypto.randomUUID(), created_at: new Date().toISOString(), schemes_matched: 0 };
  ...
  const handleTextSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (textInput.trim() && phase === 'questions') processAnswer(textInput.trim());
  };
  ...
  const startListening = () => {
    ...
    recognition.lang = speechCode; recognition.continuous = false; recognition.interimResults = false;
  ...
  const handleDismiss = () => { window.speechSynthesis?.cancel(); setIsVisible(false); };
        ...
        <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-secondary-foreground/70">
                  {phase === 'language' ? 'Choose Language' : phase === 'done' || phase === 'saving' ? '✓ Saving...' : `Question ${currentStep + 1}/7`}
                </p>
              </div>
            </div>
          ...
          {phase === 'questions' && (
            <div className="h-1 bg-muted shrink-0">
              <motion.div className="h-full bg-primary" animate={{ width: `${((currentStep + 1) / 7) * 100}%` }} transition={{ duration: 0.4 }} />
            </div>
          )}
                ...
                <motion.div key={msg.id} initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                  ...
                  <div className={`flex items-end gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                      msg.role === 'bot' ? 'bg-muted text-foreground rounded-bl-md' : 'bg-primary text-primary-foreground rounded-br-md'
                    }`}>
                        ...
                        <div className="grid grid-cols-2 gap-2 mt-3">
                          {LANGUAGES.map(l => (
                            <motion.button key={l.code} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                              ...
                              <Globe className="w-3.5 h-3.5 text-primary" /> {l.nativeLabel}
                            </motion.button>
                          ))}
                        </div>
                      )}
          ...
          {phase === 'questions' && (
            <div className="border-t border-border p-3 shrink-0 bg-card">
              <form onSubmit={handleTextSubmit} className="flex items-center gap-2">
                ...
                <motion.button type="submit" whileTap={{ scale: 0.9 }} disabled={!textInput.trim()}

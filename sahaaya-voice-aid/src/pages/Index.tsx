// src/pages/Index.tsx
// Updated: fetches real stats from backend for animated counters
...
const Index = () => {
  ...
  const [stats, setStats] = useState({ total_users: 25000, total_schemes: 150, total_jobs: 28 });

  // Fetch real stats from backend, fall back to defaults
  useEffect(() => {
    fetch(`${API_URL}/api/admin/stats`)
      ...
      .then(data => {
        if (data.success) {
          setStats({
            total_users: data.stats.total_users > 0 ? data.stats.total_users : 25000,
            total_schemes: data.stats.total_schemes > 0 ? data.stats.total_schemes : 150,
            total_jobs: data.stats.total_jobs > 0 ? data.stats.total_jobs : 28,
          });
        }
      })
  ...
  const handleDemo = () => {
    ...
    // Fetch demo schemes from backend
    fetch(`${API_URL}/api/match-schemes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ profile: {
        monthly_income: 8000, has_bpl_card: true, has_disability: false,
        occupation: 'farmer', state: 'Uttar Pradesh',
      }}),
    })
      .then(r => r.json())
      .then(data => {
        if (data.success) localStorage.setItem('sahaaya_schemes', JSON.stringify(data.schemes));
      })
      .catch(() => {})
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container py-12 md:py-20">
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h1 className="text-4xl md:text-6xl font-extrabold text-secondary leading-tight mb-4">{t.heroTitle}</h1>
            <p className="text-lg md:text-xl text-muted-foreground mb-2">{t.heroSubtitle}</p>
            <p className="text-base text-muted-foreground mb-8 max-w-md">{t.heroDescription}</p>
            <div className="flex flex-wrap gap-3">
              ...
              <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={handleDemo}
      ...
      <section className="bg-secondary py-10">
        <div className="container grid grid-cols-3 gap-4 text-center">
          {[
            { icon: Award,  label: t.schemesAvailable, value: stats.total_schemes },
            { icon: Users,  label: t.usersHelped,       value: stats.total_users  },
            { icon: MapPin, label: t.statesCovered,     value: 28                 },
          ].map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.15 }} viewport={{ once: true }} className="py-4">
      ...
      <section className="container py-16">
        <h2 className="text-2xl md:text-3xl font-bold text-secondary text-center mb-10">{t.howItWorks}</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.15 }} viewport={{ once: true }} className="bg-card rounded-card p-6 shadow-card text-center">
      ...
      <section className="container pb-16">
        <h2 className="text-2xl md:text-3xl font-bold text-secondary text-center mb-8">{t.whatWeCover}</h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[
            ...
            <motion.div key={i} whileHover={{ y: -4 }} className="bg-card rounded-card p-4 shadow-warm text-center cursor-default">

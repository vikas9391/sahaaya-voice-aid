...
const Register = () => {
  ...
  }, [isComplete, answers, navigate, lang]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="container py-4 flex items-center justify-between">
        <button onClick={() => navigate('/')} className="text-sm text-muted-foreground hover:text-foreground">← {t.back}</button>
        <span className="text-sm font-medium text-secondary">Sahaaya AI</span>
      </div>

      <div className="container mb-8">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-muted-foreground">{t.question} {step + 1} {t.of} {totalQuestions}</span>
        ...
        <div className="h-2 bg-muted rounded-full overflow-hidden">
          <motion.div className="h-full bg-primary rounded-full" initial={{ width: 0 }}
            animate={{ width: `${((step + 1) / totalQuestions) * 100}%` }} transition={{ duration: 0.4 }} />
        </div>
      </div>
      ...
      <div className="container flex-1 flex flex-col items-center justify-center max-w-lg mx-auto">
            ...
            <div className="relative">
              ...
              <motion.button whileTap={{ scale: 0.9 }} onClick={handleMicTap} disabled={isSpeaking}
                className={`relative z-10 w-32 h-32 md:w-40 md:h-40 rounded-full flex items-center justify-center transition-colors ${
                  isListening ? 'bg-destructive shadow-lg' : isSpeaking ? 'bg-muted cursor-wait' : 'bg-primary shadow-primary-glow'
                }`}>
                {isListening ? <MicOff className="w-12 h-12 text-destructive-foreground" /> : <Mic className="w-12 h-12 text-primary-foreground" />}
              </motion.button>
            </div>
        ...
        {showTextForm && (
          <form onSubmit={handleTextSubmit} className="w-full mb-8">
            <div className="flex gap-2">
              <input type={currentQuestion.type === 'number' ? 'number' : 'text'} value={textInput}
                onChange={e => setTextInput(e.target.value)} placeholder={currentQuestion.text}
        ...
        {error && (
          <div className="mt-4 bg-destructive/10 text-destructive px-4 py-3 rounded-button text-sm text-center">{error}</div>
        )}
